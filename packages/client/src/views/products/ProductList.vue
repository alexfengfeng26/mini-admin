<template>
  <div class="product-list">
    <div class="page-header"><h1 class="page-title">产品管理</h1><router-link to="/products/create" class="btn btn-primary">+ 新增产品</router-link></div>
    <div class="card"><div class="card-body">
      ...
      <table class="table"><thead><tr><th>ID</th><th>名称</th><th>编码</th><th>价格</th><th>库存</th><th>状态</th><th>操作</th></tr></thead><tbody><tr v-for="p in products" :key="p.id"><td>{{p.id}}</td><td>{{p.name}}</td><td>{{p.code}}</td><td>{{p.price||'-'}}</td><td>{{p.stock}}</td><td><span :class="['badge',p.status===1?'badge-success':p.status===2?'badge-warning':'badge-secondary']">{{p.status===1?'上架':p.status===2?'缺货':'下架'}}</span></td><td><router-link :to="`/products/${p.id}/edit`" class="btn btn-secondary">编辑</router-link><button class="btn btn-danger" @click="handleDelete(p.id)">删除</button></td></tr></tbody></table>
